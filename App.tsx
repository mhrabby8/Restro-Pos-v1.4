...
const AIDashboardInsights = ({ stats, settings, branches }: any) => {
  ...
  const generateInsight = async () => {
    ...
    try {
      ...
      const prompt = `Analyze this POS data and provide 3 brief high-impact bullet points: Revenue ${settings.currencySymbol}${stats.totalSales}, Orders ${stats.totalOrders}.`;
      ...
      setInsight("Unable to reach AI Analyst.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 space-y-8 border border-gray-100">
        <div className="text-center">
          <div className="w-20 h-20 bg-blue-600 rounded-[2rem] flex items-center justify-center text-white mx-auto shadow-xl mb-6">
            <Lock size={32} />
          </div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tight">Terminal Access</h2>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-[1.5rem] shadow-xl text-sm uppercase tracking-widest hover:bg-blue-700 transition-all active:scale-95">Open Terminal</button>
        </form>
      </div>
    </div>
  );
...
const DashboardView = ({ orders, settings, branches }: any) => {
  const stats = useMemo(() => {
    const totalSales = (orders || []).filter((o: any) => o.status !== OrderStatus.CANCELLED).reduce((acc: number, o: any) => acc + o.total, 0);
...
// --- Module: POS Terminal ---
const POSView = ({ branch, settings, addOrder, categories, menuItems, allAddons, customers, setCustomers }: any) => {
  ...
  // New States for Redemption and Promo
  ...
  const vatAmount = (subtotal * (settings.vatPercentage || 0)) / 100;
  
  // Point rules: min 30 balance, max 30% of points per tx
  const canRedeem = matchedCustomer && matchedCustomer.points >= 30;
  const pointLimit30Percent = matchedCustomer ? Math.floor(matchedCustomer.points * 0.3) : 0;
  const pointsToRedeem = (usePoints && canRedeem) ? Math.min(pointLimit30Percent, Math.floor(subtotal / settings.pointsRedeemRate)) : 0;
  ...
  const verifyPromo = () => {
    ...
    if (code === 'SAVE10') { setPromoDiscount(subtotal * 0.1); alert("10% Discount Applied!"); }
    else if (code === 'WELCOME50') { setPromoDiscount(50); alert("50 TK Discount Applied!"); }
  ...
  const addToCart = (item: MenuItem) => {
    ...
    else setCart([...cart, { id: `${item.id}-${Date.now()}`, menuItemId: item.id, name: item.name, quantity: 1, unitPrice: price, addOns: [] }]);
  };
  ...
  const handleCheckout = () => {
    ...
    addOrder({
      ...
      status: OrderStatus.PENDING, createdAt: Date.now(), userId: 'admin-1', customerName: customerInfo.name, customerPhone: customerInfo.phone
    });
    ...
    setCart([]); setIsCheckoutModalOpen(false); setCustomerInfo({ name: '', phone: '' }); setUsePoints(false); setPromoCode(''); setPromoDiscount(0);
  };

  return (
    <div className="flex h-full overflow-hidden bg-gray-50">
      <div className="flex-1 flex flex-col min-w-0 bg-white border-r h-full overflow-hidden">
        <div className="p-4 border-b flex flex-col gap-4">
          <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-2 border border-gray-100">
            <Search size={16} className="text-gray-400"/><input type="text" placeholder="Search items..." className="bg-transparent outline-none w-full text-xs font-bold" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}/>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-4 gap-4 pb-40">
          {filteredItems.map((item: MenuItem) => (
            <button key={item.id} onClick={() => addToCart(item)} className="bg-white border border-gray-100 rounded-[1.5rem] overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col text-left group">
              <div className="aspect-square bg-gray-50 overflow-hidden"><img src={item.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform" alt={item.name} /></div>
              <div className="p-3"><h4 className="font-black text-[11px] truncate">{item.name}</h4><p className="text-[10px] font-black text-blue-600 mt-1">{settings.currencySymbol}{item.price}</p></div>
            </button>
          ))}
        </div>
      </div>
      <div className="w-96 bg-white flex flex-col shrink-0">
        <div className="p-4 border-b flex justify-between items-center"><h2 className="font-black text-[10px] uppercase tracking-widest">Cart</h2><span className="bg-blue-600 text-white text-[8px] px-1.5 py-0.5 rounded">{cart.length}</span></div>
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {cart.map(item => (
            <div key={item.id} className="bg-white p-3 rounded-xl border border-gray-100 flex justify-between items-center">
              <div className="flex-1"><h4 className="text-[10px] font-black truncate">{item.name}</h4><p className="text-[9px] text-blue-600 font-bold">{settings.currencySymbol}{item.unitPrice}</p></div>
        ...
        <div className="p-4 border-t space-y-3">
          <div className="flex justify-between items-center text-[10px] font-black text-gray-400 uppercase"><span>Total</span><span className="text-gray-900 text-lg">{settings.currencySymbol}{total.toFixed(0)}</span></div>
          <button disabled={cart.length === 0} onClick={() => setIsCheckoutModalOpen(true)} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl disabled:opacity-50 text-[11px] tracking-widest uppercase active:scale-95">Checkout</button>
        </div>
      </div>
      {isCheckoutModalOpen && (
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg relative z-10 p-8 space-y-6 shadow-2xl animate-in zoom-in border border-gray-100 max-h-[95vh] overflow-y-auto no-scrollbar">
             ...
             {matchedCustomer && (
               <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-2xl flex items-center justify-between">
                 <div className="flex items-center gap-3">
                   ...
                   <div>
                     <p className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Loyalty</p>
                     <p className="text-xs font-black text-gray-900">{matchedCustomer.points.toFixed(0)} Pts ({canRedeem ? `Max ${pointLimit30Percent} redeemable` : 'Min 30 pts req.'})</p>
                   </div>
                 </div>
             ...
             <div className="space-y-1.5">
               ...
               <div className="flex gap-2">
                 <div className="flex-1 flex items-center bg-gray-50 rounded-xl px-3 py-0.5 border border-gray-100"><Ticket size={14} className="text-gray-400 mr-2"/><input type="text" className="w-full py-3 bg-transparent outline-none font-black text-xs uppercase" value={promoCode} onChange={e => setPromoCode(e.target.value)} placeholder="e.g. SAVE10"/></div>
                 <button onClick={verifyPromo} className="px-4 py-2 bg-gray-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest">Verify</button>
               </div>
             </div>
             <div className="p-5 bg-gray-900 rounded-[1.8rem] space-y-2">
               <div className="flex justify-between text-[10px] font-bold text-gray-500 uppercase"><span>Subtotal</span><span className="text-gray-300">{settings.currencySymbol}{subtotal.toLocaleString()}</span></div>
               {pointsCashValue > 0 && <div className="flex justify-between text-[10px] font-bold text-blue-400 uppercase"><span>Loyalty Discount</span><span>-{settings.currencySymbol}{pointsCashValue}</span></div>}
               {promoDiscount > 0 && <div className="flex justify-between text-[10px] font-bold text-purple-400 uppercase"><span>Promo Discount</span><span>-{settings.currencySymbol}{promoDiscount.toFixed(0)}</span></div>}
               <div className="flex justify-between text-lg font-black text-white uppercase pt-2 border-t border-white/10 mt-1"><span>Total</span><span className="text-blue-400">{settings.currencySymbol}{total.toFixed(0)}</span></div>
             </div>
    ...
    <div className="p-4 lg:p-8 space-y-6 h-full overflow-y-auto pb-32 no-scrollbar bg-gray-50/20">
        ...
        {customers.map((c: Customer) => (
          <div key={c.id} className="bg-white p-7 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col gap-6 relative group overflow-hidden">
            <div className="flex items-center gap-5">
              <div className="w-14 h-14 rounded-[1.5rem] bg-blue-50 text-blue-600 flex items-center justify-center font-black text-2xl">{c.name.charAt(0)}</div>
              ...
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100"><button onClick={() => setModal({ type: 'edit', data: c })} className="p-2 bg-gray-50 text-blue-600 rounded-lg"><Edit size={14}/></button></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-gray-50 p-3 rounded-2xl text-center"><p className="text-[7px] font-black text-gray-400 uppercase mb-1">Points</p><p className="text-xs font-black text-blue-600">{c.points.toFixed(0)}</p></div>
              <div className="bg-gray-50 p-3 rounded-2xl text-center"><p className="text-[7px] font-black text-gray-400 uppercase mb-1">Visits</p><p className="text-xs font-black text-gray-800">{c.totalOrders}</p></div>
              <div className="bg-gray-50 p-3 rounded-2xl text-center"><p className="text-[7px] font-black text-gray-400 uppercase mb-1">Spend</p><p className="text-xs font-black text-emerald-600">{settings.currencySymbol}{c.totalSpend.toFixed(0)}</p></div>
            </div>
          </div>
        ))}
      </div>
      {modal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <form onSubmit={saveCust} className="bg-white rounded-[2.5rem] w-full max-w-md relative z-10 p-8 space-y-4 shadow-2xl border border-gray-100 animate-in zoom-in">
             <h3 className="text-xl font-black uppercase tracking-widest">{modal.type === 'edit' ? 'Edit Profile' : 'Register Customer'}</h3>
             <input name="name" type="text" placeholder="Name" defaultValue={modal.data?.name} className="w-full p-4 rounded-2xl bg-gray-50 border-none font-bold text-sm outline-none focus:ring-4 focus:ring-blue-50" required />
             <input name="phone" type="text" placeholder="Phone" defaultValue={modal.data?.phone} className="w-full p-4 rounded-2xl bg-gray-50 border-none font-bold text-sm outline-none focus:ring-4 focus:ring-blue-50" required />
             {!modal.data && <p className="text-[8px] font-bold text-blue-600 uppercase ml-4">+ 10 points bonus on registration</p>}
             <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-xl">Complete Registration</button>
          </form>
        </div>
      )}
    </div>
...
const Sidebar = ({ activeTab, setActiveTab, isOpen, setIsOpen, settings, currentUser, onLogout }: any) => (
  <>
    ...
    <aside className={`fixed top-0 left-0 bottom-0 w-64 bg-white border-r border-gray-100 z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
      <div className="p-8 shrink-0 flex items-center gap-3">
        <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg"><Flame size={20} fill="currentColor" /></div>
        <div><h1 className="text-sm font-black tracking-tighter leading-none">{settings.appName}</h1><p className="text-[8px] font-black text-blue-600 uppercase tracking-widest mt-1">Enterprise Core</p></div>
      </div>
      <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1 no-scrollbar">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setIsOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${activeTab === item.id ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'text-gray-400 hover:bg-gray-50'}`}>
              {item.icon}<span className="text-[10px] font-black uppercase tracking-widest">{item.label}</span>
            </button>
          );
        })}
      </nav>
      <div className="p-4 border-t border-gray-50 shrink-0"><button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-4 rounded-2xl text-rose-500 hover:bg-rose-50 transition-all"><LogOut size={20} /><span className="text-[10px] font-black uppercase tracking-widest">Logout</span></button></div>
    </aside>
  </>
);

// --- Module: Header ---
const Header = ({ title, toggleSidebar, activeBranch, currentUser }: any) => (
  <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-8 shrink-0 z-30">
    <div className="flex items-center gap-4">
      ...
      <div><h2 className="text-lg font-black text-gray-900 uppercase tracking-tighter leading-none">{title}</h2><p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mt-1">POS: Online</p></div>
    </div>
...
export default function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'pos': return <POSView branch={activeBranch} settings={settings} addOrder={addOrder} categories={categories} menuItems={menuItems} allAddons={addons} customers={customers} setCustomers={setCustomers} />;
      case 'customers': return <CustomersView settings={settings} customers={customers} setCustomers={setCustomers} />;
      default: return <div className="p-8 text-gray-400 font-black uppercase text-xs tracking-widest text-center">Module in standard configuration mode.</div>;
    }
  };
  ...
  return (
    <div className="h-screen w-full bg-gray-50 flex overflow-hidden">
      ...
      <main className="flex-1 lg:pl-64 flex flex-col h-full relative overflow-hidden">
        <Header title={NAV_ITEMS.find(n => n.id === activeTab)?.label} toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} activeBranch={activeBranch} currentUser={currentUser} />
        <div className="flex-1 overflow-hidden relative h-full">{renderContent()}</div>
      </main>
      <style>{`@keyframes zoom-in { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: scale(1); } } .animate-in { animation-duration: 0.3s; animation-fill-mode: both; } .zoom-in { animation-name: zoom-in; } .no-scrollbar::-webkit-scrollbar { display: none; }`}</style>
    </div>
  );
}
